...
for (let header in headerData) {
  for (let genre in headerData[header]) {
...
//////////////////////////////////////////////////
const DEFAULT_BUILD_ORDER_STRING = () =>
  "index.html?c=EN&s=";
...
if (!selectedciv) {
  window.location.href = DEFAULT_BUILD_ORDER_STRING();
}
...
for (let i = 0; i < civilizations.length; i++) {
  ...
  str +=
    '><a href="index.html?c=' +
    civilizations[i].abbr +
    '">' +
    civilizations[i].civilization +
    "</a></li>";
}
...
function createRow() {
  if (typeof index !== "undefined") {
    ...
    row.innerHTML =
      '<td contenteditable="true">0:00</td><td contenteditable="true"></td>';
    getSelectedRow();
  }
  row.focus();
}
...
const tableitems = [
  ["food", "resourcefoodicon"],
  ["wood", "resourcewoodicon"],
  ["gold", "resourcegoldicon"],
  ["stone", "resourcestoneicon"],
  ["time", "timetobuild"],
  ["pop", "house"],
];
...
function formatImage(reference, value, showTooltip) {
  ...
  if (showTooltip) {
    ...
    tooltip +=
      '<img src="img/' +
      reference[selectedciv.abbr] +
      '.png"></img><br/><div class="smallIcons">';
    for (let h = 0; h < tableitems.length; h++) {
      if (reference[tableitems[h][0]]) {
        if (
          selectedciv.abbr == "CH" &&
          (reference.genre == "Building" || reference.genre == "Landmark") &&
          tableitems[h][0] == "time"
        ) {
          // Chinese building modifier
          tooltip +=
            '<img src="img/' +
            tableitems[h][1] +
            '.png">' +
            sToTime(reference[tableitems[h][0]] * buildingTimeModifier) +
            "<br/>";
        } else if (
          selectedciv.abbr == "DS" &&
          (reference.genre == "Technology" ||
            reference.genre == "Upgrade" ||
            reference.genre == "Blacksmith")
        ) {
          // Dehli upgrade modifier
          if (tableitems[h][0] == "time") {
            tooltip +=
              ...
              '.png">' +
              sToTime(
                reference[tableitems[h][0]] *
                  upgradeDSTimeModifier[reference.age - 1]
              ) +
              "<br/>";
          }
        } else {
          tooltip +=
            ...
            '.png">' +
            (tableitems[h][0] == "time"
              ? sToTime(reference[tableitems[h][0]])
              : reference[tableitems[h][0]]) +
            "<br/>";
        }
      }
    ...
    tooltip +=
      '</div></div><div class="tooltipColumn2"><header>' +
      reference.name +
      " (age: " +
      reference.age +
      "+)</header></br></br>" +
  ...
  return reference.color == "transparent"
    ? imgstr[0] +
        ...
        value +
        imgstr[2] +
        LZString.compressToEncodedURIComponent(tooltip) +
        imgstr[3] +
        reference.name +
        imgstr[6]
    : imgstr[0] +
...
function saveToURL() {
  ...
  window.history.replaceState(
    "Home",
    "AGE OF EMPIRES 4 - BUILD ORDER TOOL",
    "index.html?c=" +
      selectedciv.abbr +
      "&s=" +
...
async function loadiconsJSON() {
  ...
  document.getElementById("civilizationFlag").innerHTML =
    '<img src="img/flag' + selectedciv.abbr + '.png"></img>';
  ...
  if (buildorder) {
    ...
    let str =
      "0:00|Click here to start editing your own build (only on PC for now...)|0:00|You can add icons by dragging them from the menu: {" +
      vilval +
      '} |0:00|When you\'re happy press "Save and Copy" and share your build!|';
    window.history.replaceState(
      "Home",
      "AGE OF EMPIRES 4 - BUILD ORDER TOOL",
      "index.html?c=" + selectedciv.abbr + "&b=" + str
    );
  ...
  for (let i = 0; i < rows.length - 1; i++) {
    rows[i + 1].innerHTML =
      ...
      convertBack(buildarray[i * 2], data) +
      '</td><td contenteditable="true">' +
      convertBack(buildarray[i * 2 + 1], data) +
      "</td>";
  }
  ...
  for (let header in headerData) {
    str +=
      '<section><header class="fold">' +
      header +
      '</header><article class="boxed">';
    for (let genre in headerData[header]) {
      for (let age = 0; age < 4; age++) {
        for (let i = 0; i < headerData[header][genre][age].length; i++) {
          str += formatImage(
            data[headerData[header][genre][age][i]],
            headerData[header][genre][age][i],
            true
          );
        }
      }
  ...
  document.addEventListener("mousemove", function checkHover(e) {
    tooltipContainer.style.left =
      getWidth() > e.pageX + 400 ? e.pageX + 4 + "px" : e.pageX - 404 + "px";
    tooltipContainer.style.top = e.pageY + 2 + "px";
    const allTooltips = document.getElementsByClassName("tooltip");
    if (
      tooltipindex < 0 ||
      !allTooltips.item(tooltipindex).querySelector(":hover")
    ) {
      for (var i = 0; i < allTooltips.length; i++) {
        if (allTooltips.item(i).querySelector(":hover")) {
          console.log(allTooltips.item(i).firstChild.getAttribute("data-info"));
          tooltipBox.innerHTML = LZString.decompressFromEncodedURIComponent(
            allTooltips.item(i).firstChild.getAttribute("data-info")
          );
